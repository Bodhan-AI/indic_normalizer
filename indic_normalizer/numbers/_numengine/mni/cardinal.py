from indic_normalizer.numbers._numengine.mni.data.nums import DIRECT_DICT
from indic_normalizer.numbers._numengine.mni.data.nums import HUNDREDS_DICT
from indic_normalizer.numbers._numengine.mni.data.nums import NUMBER_SCALE_DICT
from indic_normalizer.numbers._numengine.mni.data.nums import VARIATIONS_DICT
from indic_normalizer.numbers._numengine.mni.utils import combine
...
def convert_to_text(number_str: str, index: int, prev: bool) -> list[str]:
    ...
    # Handle tens and ones place
        ...
        if index > 0 and prev:
            ...
            ones.extend(
                variation + suffix for variation in VARIATIONS_DICT.get(number_str, [])
            )
        else:
...
def process_text(
    ...
    converted_text = (
        convert_to_text(number_str.lstrip("0"), index, prev)
        if index == 0
        else combine(
            NUMBER_SCALE_DICT[str(index)],
            convert_to_text(number_str.lstrip("0"), index, prev),
        )
    )
