from indic_normalizer.numbers._numengine.kas.data.nums import DIRECT_DICT
from indic_normalizer.numbers._numengine.kas.data.nums import NUMBER_SCALE_DICT
from indic_normalizer.numbers._numengine.kas.data.nums import VARIATIONS_DICT
from indic_normalizer.numbers._numengine.kas.utils import combine
...
def convert_to_text(number_str: str) -> list[str]:
    ...
    if len(number_str) == 3 and number_str.lstrip("0"):
        ...
        scale_key = (
            NUMBER_SCALE_DICT["0"][1]
            if number_str[0] in ["8", "9"]
            else NUMBER_SCALE_DICT["0"][0]
        )
