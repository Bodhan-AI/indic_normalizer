from indic_normalizer.numbers._numengine.sat.data.nums import DIRECT_DICT
from indic_normalizer.numbers._numengine.sat.data.nums import NUMBER_SCALE_DICT
from indic_normalizer.numbers._numengine.sat.data.nums import VARIATIONS_DICT
from indic_normalizer.numbers._numengine.sat.utils import combine
...
def convert_to_text(number_str: str, index: int, number_len: int) -> list[str]:
    ...
    if len(number_str) == 3:
        if number_str.lstrip("0"):
            hundreds.append(
                " ".join(DIRECT_DICT[number_str[0]] + NUMBER_SCALE_DICT["0"])
            )
    ...
    if number_str:
        ...
        ) if number_str in VARIATIONS_DICT and index == 0 else None
...
def process_text(
    ...
    converted_text = (
        convert_to_text(number_str.lstrip("0"), index, number_len)
        if index == 0
        else combine(
            convert_to_text(number_str.lstrip("0"), index, number_len),
