from indic_normalizer.numbers._numengine.nep.data.nums import DIRECT_DICT
from indic_normalizer.numbers._numengine.nep.data.nums import NUMBER_SCALE_DICT
from indic_normalizer.numbers._numengine.nep.data.nums import VARIATIONS_DICT
from indic_normalizer.numbers._numengine.nep.utils import combine
...
def process_text(
    ...
    converted_text = (
        convert_to_text(number_str.lstrip("0"))
        if index == 0
        else combine(
            convert_to_text(number_str.lstrip("0")), NUMBER_SCALE_DICT[str(index)]
        )
    )
