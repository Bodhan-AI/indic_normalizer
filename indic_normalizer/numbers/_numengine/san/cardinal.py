import unicodedata
from indic_normalizer.numbers._numengine.san.data.nums import DIRECT_DICT
from indic_normalizer.numbers._numengine.san.data.nums import NUMBER_SCALE_DICT
from indic_normalizer.numbers._numengine.san.data.nums import PREFIX_REPLACEMENTS
from indic_normalizer.numbers._numengine.san.data.nums import VARIATIONS_DICT
from indic_normalizer.numbers._numengine.san.utils import combine
...
def combine_morphemes(primary_text: str, secondary_text: str) -> str:
    ...
    """
    suffix = next(
        (key for key in PREFIX_REPLACEMENTS if primary_text.endswith(key)), None
    )
    if suffix:
        for suffixes, replacement in PREFIX_REPLACEMENTS[suffix].items():
            matched_prefix = next(
                (val for val in suffixes if secondary_text.startswith(val)), None
            )
            if matched_prefix:
                primary_text = (
                    primary_text[: -len(suffix)] + PREFIX_REPLACEMENTS[suffix][suffixes]
                )
                if (
                    (
                        matched_prefix
                    ...
                    or (matched_prefix in ["अ", "उ"])
                    or (matched_prefix == "द" and suffix == "षट्")
                ):
...
def convert_to_text(
    number_str: str, index: int, number_len: int
    ...
    suffix = (
        "उत्तर"
        if number_len > 3 and len(number_str) == 3
        else "अधिक"
        if number_len > 3 and index == 0
        else None
    )
    ...
    if len(number_str) == 3:
        if number_str[0] != "1":
            ...
            prefixes.extend(
                variation for variation in VARIATIONS_DICT.get(number_str[0], [])
            )

        number_str = number_str[1:].lstrip("0")
        suffix = "उत्तर" if number_len > 3 else "अधिक"
        texts.extend(
            [combine_morphemes(DIRECT_DICT[number_str][0], suffix)]
            if number_str
            else []
        )
        texts.extend(
            [
                combine_morphemes(variation, suffix)
                for variation in VARIATIONS_DICT.get(number_str, [])
            ]
        )
...
def process_text(
    number_str: str, texts: list[str], index: int, number_len
) -> list[str]:
    """
    ...
    if index == 0:
        if len(number_str) == 3:
            ...
            combined_prefixes = [
                combine_morphemes(prefix, NUMBER_SCALE_DICT[str(index)][0])
                for prefix in prefixes
            ] or [scale]
            converted_text = combine(
                converted_text,
                [
                    combine_morphemes(combined_prefix, suffix)
                    for combined_prefix in combined_prefixes
                ],
                "",
            )
        else:
        ...
        prefixes = (
            convert_to_text(number_str, index, number_len) if number_str != "1" else []
        )
        suffix = "अधिक" if texts else ""
        combined_prefixes = [
            combine_morphemes(prefix, scale) for prefix in prefixes
        ] or [scale]
        converted_text = [
            combine_morphemes(combined_prefix, suffix)
            for combined_prefix in combined_prefixes
