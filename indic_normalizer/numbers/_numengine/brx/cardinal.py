from indic_normalizer.numbers._numengine.brx.data.nums import DIRECT_DICT
from indic_normalizer.numbers._numengine.brx.data.nums import NUMBER_SCALE_DICT
from indic_normalizer.numbers._numengine.brx.data.nums import VARIATIONS_DICT
from indic_normalizer.numbers._numengine.brx.utils import combine
...
def convert_to_text(number_str: str, number_len: int) -> list[str]:
    ...
    if len(number_str) == 3 and number_str.lstrip("0"):
        if number_str in DIRECT_DICT.keys() and number_len == 3:
            hundreds.append(DIRECT_DICT[number_str][0])
        ...
        if number_str[0] in VARIATIONS_DICT:
            hundreds.extend(
                [
                    variation + NUMBER_SCALE_DICT["0"][0]
...
def process_text(
    ...
    sep = " " if len(number_str.lstrip("0")) == 2 or index >= 2 else ""
    converted_text = (
        convert_to_text(number_str.lstrip("0"), number_len)
        if index == 0
        else combine(
            convert_to_text(number_str.lstrip("0"), number_len),
