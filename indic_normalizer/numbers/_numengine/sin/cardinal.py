from indic_normalizer.numbers._numengine.sin.data.nums import DIRECT_DICT
from indic_normalizer.numbers._numengine.sin.data.nums import NUMBER_SCALE_DICT
from indic_normalizer.numbers._numengine.sin.data.nums import VARIATIONS_DICT
from indic_normalizer.numbers._numengine.sin.utils import combine
...
def convert_to_text(number_str: str, index: int) -> list[str]:
    ...
    if len(number_str) == 3 and number_str.lstrip("0"):
        prefix = (
            [DIRECT_DICT[number_str[0]][1]]
            if number_str[0] == "1"
            else DIRECT_DICT[number_str[0]]
        )
        ...
        if number_str[0] in VARIATIONS_DICT:
            hundreds.extend(
                [
                    " ".join([variation, NUMBER_SCALE_DICT["0"][0]])
    ...
    if number_str:
        ...
        ones.extend(
            VARIATIONS_DICT[number_str]
        ) if number_str in VARIATIONS_DICT else None

    return combine(hundreds, ones)


def process_text(
    number_str: str, texts: list[str], index: int, number_len: int
) -> list[str]:
    """
    ...
    condition = (
        number_str.lstrip("0") == "1"
        and texts == []
        and number_len == (index - 1) + 3 + index
        and index != 2
    )
    scale_idx = 1 if condition else 0

    converted_text = (
        convert_to_text(number_str.lstrip("0"), index)
        if index == 0
        else combine(
            convert_to_text(number_str.lstrip("0"), index),
            [NUMBER_SCALE_DICT[str(index)][scale_idx]],
        )
    )
